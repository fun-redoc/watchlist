...
export default function YFinApp({show,checkFavorite, showBuy, addToWatch, additionalHeaderElements}:
                                {show:boolean} &
                                {checkFavorite?:(symbol:string)=>boolean} &
  ...
  return (
    <MainLayout show={show}>
...

        {
          ...
          :
          <StockList<YFinElem> 
            list={yfinCtx.queryResult ?.map((a,i)=>{return {...a, i:i, fav:((checkFavorite && checkFavorite(a.symbol)) || false)}})} 
            stockRepresentation={(s) => <StockItemRepresentation kind={s.typeDisp} symbol={s.symbol} name={s.name} fav={s.fav} /> }
            onSelect={async (s)=>{
                              setSelected("loading")
                              //const assetDetails = await yfinCtx.getDetails(s as YFinAutocompleteResult)
                              //setSelected(s)
                              //setDetails(assetDetails)
                              yfinCtx.getDetails(s as YFinAutocompleteResult)
                                .then(assetDetails => {
                                  setSelected(s)
                                  setDetails(assetDetails)
                                })
        ...
        }
        {
          selected ? 
            selected === "loading" ?
              ...
              :
                showBuy ?
                  <StockDetail value={{...selected , ...details}}
