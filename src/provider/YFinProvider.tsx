...
import useYFinApi, { YFinAutocompleteResult, YFinQuoteResult } from "../hooks/useYFinApi";
...
function useYFinProvider():{
    ...
    async function searchYFin(q?:string | null):Promise<YFinAutocompleteResult[]> {
    if(!q) return []
    ...
    type TAction = { action:"setQueryReslut", param:TState["queryResult"] } |
                   { action:"setQuery",    param:TState["query"] } 
    
    // Statemanagement state transition 
    const [{queryResult, query}, dispatch] = useReducer((state:TState, action:TAction) => {
        switch(action.action) {
            case "setQueryReslut": return {...state, queryResult:action.param }
    ...
    useEffect(() => {
        ...
        searchYFin(query)
            .then(list => dispatch({action:"setQueryReslut", param:list}))
    },[query])
    ...
    const getDetails = useCallback( (asset:YFinAutocompleteResult) =>  {
...
const YFinContext = createContext<ReturnType<typeof useYFinProvider>>({} as ReturnType<typeof useYFinProvider>)
