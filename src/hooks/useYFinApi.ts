...
export default function useYFinApi(useMock=true):{
    autocomplete:(q:string | null, abortController?:AbortController) => Promise<YFinAutocompleteResult[]>,
    getAsset:(symbol:string, abortController?:AbortController) => Promise<YFinQuoteResult>,
    getAssetBatch:{MAX_BATCH_SIZE:number, fetchBatch:(batchOfSymbols:string[],abortController?:AbortController|undefined) => Promise<YFinQuoteResult[]>}
} {
    const settingsCtx = useSettings()


    const api_autocomplete = useCallback(
    async (q:string | null, abortController?:AbortController):Promise<YFinAutocompleteResult[]> => {
        ...
        if(settingsCtx?.apiKey) {
            ...
            throw new Error("No api key for yfinance found in settings. Edit your settings and try again.")
        }
        const service = 'https://yfapi.net/v6/finance/autocomplete?region=DE&lang=de'
        ...
        try {
            ...
            if(response.status === 401 || response.status === 403) {
                throw {status:response.status, statusText:response.statusText} as AuthError
            }
            if(!response.ok) {
                throw {status:response.status, statusText:response.statusText} as ApiError
            }
    ...
    if(useMock) {
        return {autocomplete:mock_autocomplete, getAsset:(symbol:string, abortController?:AbortController)=> mock_getAsset(symbol, abortController, 1500), getAssetBatch:mock_getAssetBatch()}
    } else {
        return {autocomplete:api_autocomplete, getAsset:()=>{throw new Error("not yet implemented")}, getAssetBatch:{MAX_BATCH_SIZE:0, fetchBatch:()=>{throw new Error("not yet implemented")}}}
    }
}
...
export async function mock_autocomplete(q:string | null, abortController?:AbortController):Promise<YFinAutocompleteResult[]> {
...
export async function mock_getAsset(symbol:string, abortController?:AbortController, delay=0):Promise<YFinQuoteResult> {
...
export function mock_getAssetBatch() {
    ...
    async function fetchBatch(batchOfSymbols:string[],abortController?:AbortController):Promise<YFinQuoteResult[]> {
        console.warn("unsing mock getAsstetBatch")
        if(batchOfSymbols.length>MAX_BATCH_SIZE) {
            console.error(`batch is too big, max ${MAX_BATCH_SIZE} expected but got ${batchOfSymbols.length}`)
            throw new Error("An error happened, try again later. For details check application log")
        } 
        return Promise.all( batchOfSymbols.flatMap(async (s)=> {
            return new Promise<YFinQuoteResult>((resolve, reject) => {
                const hTimeout = setTimeout(() => { // simulate netword lattency on each batch
                    mock_getAsset(s, abortController)
                    .then(success=>resolve(success), fail=>reject(fail))
                }, 300)
                abortController?.signal.addEventListener("abort", () => clearTimeout(hTimeout))
            })
         } ))
    }
